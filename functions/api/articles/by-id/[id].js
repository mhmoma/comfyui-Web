...
export async function onRequestPatch(context) {
  ...
  const row = await getRowById(db, params.id);
  if (!row) return json(404, { error: '文章不存在' });
  ...
  try {
    ...
    ).bind(title, summary, cover_url, content, category, tags, author, status, published_at, now, params.id).run();

    const updated = await getRowById(db, params.id);
...
export async function onRequestDelete(context) {
  ...
  try {
    const result = await db.prepare('DELETE FROM articles WHERE id = ?').bind(params.id).run();
